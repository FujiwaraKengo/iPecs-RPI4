...
#do sudo apt install ntp and pip install ntplib
#import ntplib
...
#         os.system(command)
    ...
    def handle_updates(self):
        while True:
            ...
            #Send the data's of Local To Firebase
...
#----------------------------------------------------------------------------------------#
            #Fetches data CreditCriticalLevel, CurrentCredit, and ElectricityPrice of Room-1 of firebase to local json
            if FirebaseData is not None:
                if FirebaseData != self.lastFirebaseData:
                    
                    # Update key access based on the new structure
                    try:
                        credit_critical_level = FirebaseData["CreditCriticalLevel"]
                        current_credit = FirebaseData["CurrentCredit"]
                        electricity_price = FirebaseData["ElectricityPrice"]
                    except KeyError as e:
                        print(f"KeyError: {e}. Check the structure of FirebaseData.")

                    # Create a dictionary with only the fields you want to update
                    required_fields = {
                        "CreditCriticalLevel": credit_critical_level,
                        "CurrentCredit": current_credit,
                        "ElectricityPrice": electricity_price,
                    }

                    # Update the local JSON data with the extracted values
